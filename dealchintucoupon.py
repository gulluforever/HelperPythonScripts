    ...
    try:
        ...
        for box in boxes:
            try:
                ...
                dealtitle = dealtitle.replace('Amazon','')
                dealtitle = dealtitle.replace('amazon','')
                ...
                try:
                    ...
                    dealshopurl = box.find('a',id='A_17')['href'].replace('http://dealschintu.com/url.php?url=','')                    
                if '&uid=' in dealshopurl:
                    dealshopurl = dealshopurl[:dealshopurl.index('&uid=')]
                coupon = ''
                #print box
                try:
                    coupon = box.find('span',id="SPAN_16").text
                    #print 'ERROR AT GETTING COUPON'
                ...
                if 'threads' in dealurl:
                    ...
                    post.custom_fields.append({
                        ...
                        'value' : 'http://couponsmama.in/wp-content/uploads/2015/08/amazonbrand.jpg'})
                    post.custom_fields.append({
...
def createdealpages():
    global pageurls
    url = "http://dealschintu.com/pages/homepage/?pn=12&cat=902"
    for i in range(1,12):
        url = "http://dealschintu.com/pages/homepage/?pn="+str(i)+"&cat=902"
        pageurls.append(url)
    pageurls.reverse()
    for pageurl in pageurls:
        scrapedeals(pageurl)
        print("%s in DONE!" %(pageurl))
wp = Client('http://couponsmama.in/xmlrpc.php', 'admin', 'genius123.')
createdealpages()
